...
const SearchPage: React.FC = () => {
  ...
  const bottomPad =
    keyboardInset > 12 ? keyboardInset + 16 + 72 : undefined;
  ...
  return (
    <div className="space-y-12" style={bottomPad !== undefined ? { paddingBottom: bottomPad } : undefined}>
      <div className="space-y-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-2">
            <h1 className="text-3xl sm:text-4xl md:text-5xl font-display font-black tracking-tighter uppercase">
              RECHERCHE <span className="text-[#e50914]">AVANCÉE</span>
            </h1>
            <p className="text-gray-500 text-sm md:text-base font-medium">
              Explorez des milliers de chaînes avec des filtres précis.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="relative">
              ...
              >
                ...
                {countries
                  ...
                  .map((country) => (
                    <option key={country.code} value={country.code.toLowerCase()} className="bg-[#141414] text-white">
                      {country.name}
                    </option>
                  ))}
              </select>
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <select className={selectFieldClass} defaultValue="all" aria-label="Qualité vidéo">
                <option value="all" className="bg-[#141414] text-white">
                  Toutes qualités
                </option>
                <option value="hd" className="bg-[#141414] text-white">
                  Haute Définition (HD)
                </option>
                <option value="sd" className="bg-[#141414] text-white">
      ...
      <div
        ...
        className="min-h-[280px] scroll-mt-28 md:scroll-mt-24"
        aria-label="Résultats de recherche"
      >
        {filteredChannels.length > 0 ? (
          <>
            <p className="text-[10px] font-black uppercase tracking-widest text-gray-500 mb-4 md:hidden">
              {displayedChannels.length} affichée{displayedChannels.length > 1 ? 's' : ''} sur {filteredChannels.length}
              {visibleCount < filteredChannels.length ? ' — fais défiler ou « Voir plus »' : ''}
            </p>
                    ...
                    <div className="aspect-video relative overflow-hidden bg-gradient-to-br from-white/5 to-transparent">
                      {channel.logo ? (
                        <img
                          src={channel.logo}
                          alt={channel.name}
                          className="w-full h-full object-contain p-8 group-hover:blur-[2px] transition-all duration-500"
                        />
                      ) : (
                      ...
                      <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-4">
                        <Link
                          ...
                          className="bg-white text-black p-4 rounded-full scale-75 group-hover:scale-100 transition-all hover:bg-[#e50914] hover:text-white shadow-2xl"
                        >
                      ...
                      >
                        <Heart
                          size={16}
                          fill={(profile?.favorites ?? []).includes(channel.id) ? 'currentColor' : 'none'}
                        />
                      </button>

                      <div className="absolute top-4 left-4">
                        <span className="bg-[#e50914] text-white px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-widest shadow-lg">
                          LIVE
                        </span>
                      </div>
                    </div>
                      ...
                      <div className="min-w-0 pr-2">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          <span className="text-[9px] text-gray-500 uppercase font-black tracking-widest truncate">
                            {channel.categories[0] || 'Général'}
                          </span>
                        </div>
                      </div>
                      <Link
                        to={`/channel/${channel.id}`}
                        className="p-2 bg-white/5 rounded text-gray-400 hover:text-white transition-colors shrink-0"
                      >
            ...
            {visibleCount < filteredChannels.length && (
              <div className="mt-10 flex flex-col items-center gap-3">
                <p className="text-gray-500 text-[10px] font-black uppercase tracking-widest text-center max-w-md">
                  Affichage de {displayedChannels.length} sur {filteredChannels.length} résultats
                </p>
                <button
                  type="button"
                  onClick={() => setVisibleCount((c) => c + CHANNELS_PAGE_SIZE)}
                  className="bg-white/5 border border-white/15 text-white px-8 py-3.5 rounded-xl font-black text-[10px] uppercase tracking-widest hover:border-[#e50914]/50 hover:bg-[#e50914]/10 hover:text-white transition-all active:scale-[0.98] shadow-lg"
                >
                  Voir plus
                </button>
              </div>
          ...
          <div className="flex flex-col items-center justify-center py-32 text-center space-y-6 glass rounded-2xl border border-dashed border-white/10">
            ...
            <div className="space-y-2">
              ...
              <p className="text-gray-500 max-w-sm text-sm font-medium">
                Nous n&apos;avons trouvé aucune chaîne correspondant à vos critères. Essayez d&apos;ajuster les filtres ou la
                recherche.
              </p>
            </div>
            <button
